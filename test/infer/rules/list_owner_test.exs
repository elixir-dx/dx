defmodule Infer.Rules.ListOwnerTest do
  ...
  test "created by list owner", %{user: user, list: list} do
    ...
    assert Infer.load!(task, :created_by_list_owner?, extra_rules: Rules)
  end
  ...
  test "created by other user", %{list: list} do
    ...
    assert Infer.load!(task, :created_by_list_owner?, extra_rules: Rules) == false
  end
end
