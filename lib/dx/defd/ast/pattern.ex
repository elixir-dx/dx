defmodule Dx.Defd.Ast.Pattern do
  ...
  def load_relevant_scopes({input_ast, state}, data_reqs) do
    load_relevant_scopes(input_ast, data_reqs, state)
  end

  def load_relevant_scopes(list, data_reqs, state) when is_list(list) do
    if elem_reqs = data_reqs[:__list__] do
      load_relevant_scopes_in_list(list, elem_reqs, state)
    else
      {list, state}
    end
  end

  def load_relevant_scopes({elem0, elem1} = ast, data_reqs, state) do
    if elem_reqs = data_reqs[{:__tuple__, 2}] do
      {[elem0, elem1], state} = load_relevant_scopes_in_list([elem0, elem1], elem_reqs, state)
  ...
  def load_relevant_scopes({:{}, meta, elems} = ast, data_reqs, state) do
    if elem_reqs = data_reqs[{:__tuple__, length(elems)}] do
      {elems, state} = load_relevant_scopes_in_list(elems, elem_reqs, state)
  ...
  def load_relevant_scopes(input_ast, _data_reqs, state) do
  ...
  defp load_relevant_scopes_in_list(list, data_reqs_by_index, state) do
    list
    |> Enum.with_index()
    |> Enum.map_reduce(state, fn {elem, index}, state ->
      data_reqs = Map.get(data_reqs_by_index, index, %{})
      load_relevant_scopes(elem, data_reqs, state)
    end)
  end
