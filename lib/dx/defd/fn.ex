defmodule Dx.Defd.Fn do
  ...
  def normalize({:fn, meta, [{:->, _meta, [args, _body]} | _] = clauses} = orig_ast, state) do
    arity = length(args)
      ...
      case final_args_ast do
          ...
          clauses =
            Enum.map(case_clauses, fn
              {:->, meta, [[wrapped_args], body]} ->
                {:->, meta, [Ast.unwrap_args(wrapped_args, args), {:ok, body}]}
            end)
        ...
        {:case, _meta, [^case_subject, [do: case_clauses]]} ->
          clauses =
            Enum.map(case_clauses, fn
              {:->, meta, [[wrapped_args], body]} ->
                {:->, meta, [Ast.unwrap_args(wrapped_args, args), body]}
            end)
