defmodule Dx.Defd.Fn do
  ...
  def normalize({:fn, meta, [{:->, _meta, [args, _body]} | _] = clauses} = ast, state) do
    arity = length(args)
      ...
      [
        ...
        {:%{}, [line: line],
         [
           ...
           fun: {:fn, meta, [{:->, meta, [generated_args, ast]}]},
           ok_fun: if(ok?, do: {:fn, meta, [{:->, meta, [generated_args, ok_body]}]}),
           final_args_fun: {:fn, meta, [{:->, meta, [generated_args, final_args_ast]}]},
           final_args_ok_fun:
             if(final_args_ok?,
               do: {:fn, meta, [{:->, meta, [generated_args, final_args_ok_body]}]}
             ),
           scope:
             if(scope_ast == :error,
               do:
                 Dx.Scope.Compiler.prepend_fallback_assigns(
                   {:fn, meta, [{:->, meta, [generated_args, ast]}]},
                   state
                 ),
